...
#    print STDERR "remote request for download from admin.mediacloud.org.\n URL: " . 'http://admin.mediacloud.org/admin/downloads/view/' . $download->{downloads_id} . "\n";
    ...
    my $username = CConfig->get_config->{ mediawords }->{ fetch_remote_content_username };
    my $password = CConfig->get_config->{ mediawords }->{ fetch_remote_content_password };
    my $url      = CConfig->get_config->{ mediawords }->{ fetch_remote_content_url };
...
sub fetch_content
{
    ...
    my $fetch_remote = CConfig->get_config->{ mediawords }->{ fetch_remote_content } || 'no';
